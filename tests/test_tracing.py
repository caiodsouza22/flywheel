"""Tests for flywheel.tracing."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.tracing import Tracing
from flywheel.models import Job, JobState


def test_tracing_snapshot_and_score() -> None:
    comp = Tracing(name="tracing")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "tracing"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "tracing"


def test_tracing_incr_and_history() -> None:
    comp = Tracing()
