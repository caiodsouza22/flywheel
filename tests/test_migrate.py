"""Tests for flywheel.migrate."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.migrate import Migrate
from flywheel.models import Job, JobState


def test_migrate_snapshot_and_score() -> None:
    comp = Migrate(name="migrate")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "migrate"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "migrate"


def test_migrate_incr_and_history() -> None:
    comp = Migrate()
