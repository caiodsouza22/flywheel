"""Tests for flywheel.fanout."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.fanout import Fanout
from flywheel.models import Job, JobState


def test_fanout_snapshot_and_score() -> None:
    comp = Fanout(name="fanout")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "fanout"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "fanout"


def test_fanout_incr_and_history() -> None:
    comp = Fanout()
