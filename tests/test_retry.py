"""Tests for flywheel.retry."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.retry import Retry
from flywheel.models import Job, JobState


def test_retry_snapshot_and_score() -> None:
    comp = Retry(name="retry")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "retry"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "retry"


def test_retry_incr_and_history() -> None:
    comp = Retry()
