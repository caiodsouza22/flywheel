"""Tests for flywheel.watchdog."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.watchdog import Watchdog
from flywheel.models import Job, JobState


def test_watchdog_snapshot_and_score() -> None:
    comp = Watchdog(name="watchdog")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "watchdog"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "watchdog"


def test_watchdog_incr_and_history() -> None:
    comp = Watchdog()
