"""Tests for flywheel.hooks."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.hooks import Hooks
from flywheel.models import Job, JobState


def test_hooks_snapshot_and_score() -> None:
    comp = Hooks(name="hooks")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "hooks"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "hooks"


def test_hooks_incr_and_history() -> None:
    comp = Hooks()
