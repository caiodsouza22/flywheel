"""Tests for flywheel.store."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.store import Store
from flywheel.models import Job, JobState


def test_store_snapshot_and_score() -> None:
    comp = Store(name="store")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "store"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "store"


def test_store_incr_and_history() -> None:
    comp = Store()
