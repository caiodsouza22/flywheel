"""Tests for flywheel.sharding."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.sharding import Sharding
from flywheel.models import Job, JobState


def test_sharding_snapshot_and_score() -> None:
    comp = Sharding(name="sharding")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "sharding"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "sharding"


def test_sharding_incr_and_history() -> None:
    comp = Sharding()
