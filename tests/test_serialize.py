"""Tests for flywheel.serialize."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.serialize import Serialize
from flywheel.models import Job, JobState


def test_serialize_snapshot_and_score() -> None:
    comp = Serialize(name="serialize")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "serialize"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "serialize"


def test_serialize_incr_and_history() -> None:
    comp = Serialize()
