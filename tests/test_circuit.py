"""Tests for flywheel.circuit."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.circuit import Circuit
from flywheel.models import Job, JobState


def test_circuit_snapshot_and_score() -> None:
    comp = Circuit(name="circuit")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "circuit"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "circuit"


def test_circuit_incr_and_history() -> None:
    comp = Circuit()
