"""Tests for flywheel.protocols."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.protocols import Protocols
from flywheel.models import Job, JobState


def test_protocols_snapshot_and_score() -> None:
    comp = Protocols(name="protocols")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "protocols"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "protocols"


def test_protocols_incr_and_history() -> None:
    comp = Protocols()
