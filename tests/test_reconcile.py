"""Tests for flywheel.reconcile."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.reconcile import Reconcile
from flywheel.models import Job, JobState


def test_reconcile_snapshot_and_score() -> None:
    comp = Reconcile(name="reconcile")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "reconcile"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "reconcile"


def test_reconcile_incr_and_history() -> None:
    comp = Reconcile()
