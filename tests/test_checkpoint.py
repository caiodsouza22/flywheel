"""Tests for flywheel.checkpoint."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.checkpoint import Checkpoint
from flywheel.models import Job, JobState


def test_checkpoint_snapshot_and_score() -> None:
    comp = Checkpoint(name="checkpoint")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    assert comp.explain(job)["component"] == "checkpoint"


def test_checkpoint_incr_and_history() -> None:
    comp = Checkpoint()
