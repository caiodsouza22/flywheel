...
def test_priority_snapshot_and_score() -> None:
    comp = Priority(name="priority")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "priority"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "priority"


def test_priority_incr_and_history() -> None:
    comp = Priority()
