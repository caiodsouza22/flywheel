"""Tests for flywheel.simulate."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.simulate import Simulate
from flywheel.models import Job, JobState


def test_simulate_snapshot_and_score() -> None:
    comp = Simulate(name="simulate")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "simulate"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "simulate"


def test_simulate_incr_and_history() -> None:
    comp = Simulate()
