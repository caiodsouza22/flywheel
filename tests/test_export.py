"""Tests for flywheel.export."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.export import Export
from flywheel.models import Job, JobState


def test_export_snapshot_and_score() -> None:
    comp = Export(name="export")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "export"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "export"


def test_export_incr_and_history() -> None:
    comp = Export()
