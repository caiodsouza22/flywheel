"""Tests for flywheel.journal."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.journal import Journal
from flywheel.models import Job, JobState


def test_journal_snapshot_and_score() -> None:
    comp = Journal(name="journal")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "journal"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "journal"


def test_journal_incr_and_history() -> None:
    comp = Journal()
