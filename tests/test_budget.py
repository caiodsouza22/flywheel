"""Tests for flywheel.budget."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.budget import Budget
from flywheel.models import Job, JobState


def test_budget_snapshot_and_score() -> None:
    comp = Budget(name="budget")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "budget"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "budget"


def test_budget_incr_and_history() -> None:
    comp = Budget()
