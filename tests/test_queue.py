"""Tests for flywheel.queue."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.queue import Queue
from flywheel.models import Job, JobState


def test_queue_snapshot_and_score() -> None:
    comp = Queue(name="queue")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "queue"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "queue"


def test_queue_incr_and_history() -> None:
    comp = Queue()
