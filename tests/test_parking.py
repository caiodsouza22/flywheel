"""Tests for flywheel.parking."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.parking import Parking
from flywheel.models import Job, JobState


def test_parking_snapshot_and_score() -> None:
    comp = Parking(name="parking")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "parking"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "parking"


def test_parking_incr_and_history() -> None:
    comp = Parking()
