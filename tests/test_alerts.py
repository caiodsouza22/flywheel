"""Tests for flywheel.alerts."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.alerts import Alerts
from flywheel.models import Job, JobState


def test_alerts_snapshot_and_score() -> None:
    comp = Alerts(name="alerts")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "alerts"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "alerts"


def test_alerts_incr_and_history() -> None:
    comp = Alerts()
