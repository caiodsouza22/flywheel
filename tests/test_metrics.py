...
def test_metrics_snapshot_and_score() -> None:
    ...
    snap = comp.snapshot()
    assert snap["name"] == "metrics"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "metrics"


def test_metrics_incr_and_history() -> None:
    comp = Metrics()
