"""Tests for flywheel.fairness."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.fairness import Fairness
from flywheel.models import Job, JobState


def test_fairness_snapshot_and_score() -> None:
    comp = Fairness(name="fairness")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "fairness"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "fairness"


def test_fairness_incr_and_history() -> None:
    comp = Fairness()
