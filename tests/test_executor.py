"""Tests for flywheel.executor."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.executor import Executor
from flywheel.models import Job, JobState


def test_executor_snapshot_and_score() -> None:
    comp = Executor(name="executor")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "executor"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "executor"


def test_executor_incr_and_history() -> None:
    comp = Executor()
