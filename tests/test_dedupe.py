"""Tests for flywheel.dedupe."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.dedupe import Dedupe
from flywheel.models import Job, JobState


def test_dedupe_snapshot_and_score() -> None:
    comp = Dedupe(name="dedupe")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "dedupe"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "dedupe"


def test_dedupe_incr_and_history() -> None:
    comp = Dedupe()
