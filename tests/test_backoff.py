"""Tests for flywheel.backoff."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.backoff import Backoff
from flywheel.models import Job, JobState


def test_backoff_snapshot_and_score() -> None:
    comp = Backoff(name="backoff")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "backoff"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "backoff"


def test_backoff_incr_and_history() -> None:
    comp = Backoff()
