"""Tests for flywheel.events."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.events import Events
from flywheel.models import Job, JobState


def test_events_snapshot_and_score() -> None:
    comp = Events(name="events")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "events"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "events"


def test_events_incr_and_history() -> None:
    comp = Events()
