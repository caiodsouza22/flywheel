"""Tests for flywheel.util."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.util import Util
from flywheel.models import Job, JobState


def test_util_snapshot_and_score() -> None:
    comp = Util(name="util")
    job = Job(
        job_id="j1",
        queue="default",
    ...
    snap = comp.snapshot()
    assert snap["name"] == "util"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "util"


def test_util_incr_and_history() -> None:
    comp = Util()
