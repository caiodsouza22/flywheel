"""Tests for flywheel.affinity."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.affinity import Affinity
from flywheel.models import Job, JobState


def test_affinity_snapshot_and_score() -> None:
    comp = Affinity(name="affinity")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "affinity"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "affinity"


def test_affinity_incr_and_history() -> None:
    comp = Affinity()
