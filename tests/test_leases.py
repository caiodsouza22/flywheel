"""Tests for flywheel.leases."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.leases import Leases
from flywheel.models import Job, JobState


def test_leases_snapshot_and_score() -> None:
    comp = Leases(name="leases")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "leases"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "leases"


def test_leases_incr_and_history() -> None:
    comp = Leases()
