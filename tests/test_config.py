...
def test_config_snapshot_and_score() -> None:
    comp = Config(name="config")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "config"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "config"


def test_config_incr_and_history() -> None:
    comp = Config()
