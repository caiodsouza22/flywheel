"""Tests for flywheel.result."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.result import Result
from flywheel.models import Job, JobState


def test_result_snapshot_and_score() -> None:
    comp = Result(name="result")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "result"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "result"


def test_result_incr_and_history() -> None:
    comp = Result()
