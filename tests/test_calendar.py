"""Tests for flywheel.calendar."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.calendar import Calendar
from flywheel.models import Job, JobState


def test_calendar_snapshot_and_score() -> None:
    comp = Calendar(name="calendar")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "calendar"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "calendar"


def test_calendar_incr_and_history() -> None:
    comp = Calendar()
