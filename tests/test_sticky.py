"""Tests for flywheel.sticky."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.sticky import Sticky
from flywheel.models import Job, JobState


def test_sticky_snapshot_and_score() -> None:
    comp = Sticky(name="sticky")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "sticky"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "sticky"


def test_sticky_incr_and_history() -> None:
    comp = Sticky()
