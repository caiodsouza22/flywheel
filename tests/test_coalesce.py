"""Tests for flywheel.coalesce."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.coalesce import Coalesce
from flywheel.models import Job, JobState


def test_coalesce_snapshot_and_score() -> None:
    comp = Coalesce(name="coalesce")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "coalesce"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "coalesce"


def test_coalesce_incr_and_history() -> None:
    comp = Coalesce()
