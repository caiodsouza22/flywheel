"""Tests for flywheel.batch."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.batch import Batch
from flywheel.models import Job, JobState


def test_batch_snapshot_and_score() -> None:
    comp = Batch(name="batch")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "batch"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "batch"


def test_batch_incr_and_history() -> None:
    comp = Batch()
