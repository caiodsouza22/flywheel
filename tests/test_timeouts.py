"""Tests for flywheel.timeouts."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.timeouts import Timeouts
from flywheel.models import Job, JobState


def test_timeouts_snapshot_and_score() -> None:
    comp = Timeouts(name="timeouts")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "timeouts"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "timeouts"


def test_timeouts_incr_and_history() -> None:
    comp = Timeouts()
