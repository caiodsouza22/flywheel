"""Tests for flywheel.worker."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.worker import Worker
from flywheel.models import Job, JobState


def test_worker_snapshot_and_score() -> None:
    comp = Worker(name="worker")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "worker"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "worker"


def test_worker_incr_and_history() -> None:
    comp = Worker()
