"""Tests for flywheel.heartbeat."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.heartbeat import Heartbeat
from flywheel.models import Job, JobState


def test_heartbeat_snapshot_and_score() -> None:
    comp = Heartbeat(name="heartbeat")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "heartbeat"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "heartbeat"


def test_heartbeat_incr_and_history() -> None:
    comp = Heartbeat()
