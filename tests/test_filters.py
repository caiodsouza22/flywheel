"""Tests for flywheel.filters."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.filters import Filters
from flywheel.models import Job, JobState


def test_filters_snapshot_and_score() -> None:
    comp = Filters(name="filters")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "filters"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "filters"


def test_filters_incr_and_history() -> None:
    comp = Filters()
