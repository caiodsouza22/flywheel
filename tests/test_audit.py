"""Tests for flywheel.audit."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.audit import Audit
from flywheel.models import Job, JobState


def test_audit_snapshot_and_score() -> None:
    comp = Audit(name="audit")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "audit"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "audit"


def test_audit_incr_and_history() -> None:
    comp = Audit()
