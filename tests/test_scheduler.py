"""Tests for flywheel.scheduler."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.scheduler import Scheduler
from flywheel.models import Job, JobState


def test_scheduler_snapshot_and_score() -> None:
    comp = Scheduler(name="scheduler")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "scheduler"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "scheduler"


def test_scheduler_incr_and_history() -> None:
    comp = Scheduler()
