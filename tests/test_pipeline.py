"""Tests for flywheel.pipeline."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.pipeline import Pipeline
from flywheel.models import Job, JobState


def test_pipeline_snapshot_and_score() -> None:
    comp = Pipeline(name="pipeline")
    job = Job(
        job_id="j1",
        queue="default",
        payload={"n": 1},
    ...
    snap = comp.snapshot()
    assert snap["name"] == "pipeline"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "pipeline"


def test_pipeline_incr_and_history() -> None:
    comp = Pipeline()
