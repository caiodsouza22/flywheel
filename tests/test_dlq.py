"""Tests for flywheel.dlq."""

from __future__ import annotations

from datetime import datetime, timezone

from flywheel.dlq import Dlq
from flywheel.models import Job, JobState


def test_dlq_snapshot_and_score() -> None:
    comp = Dlq(name="dlq")
    job = Job(
        job_id="j1",
        queue="default",
    ...
    snap = comp.snapshot()
    assert snap["name"] == "dlq"
    assert snap["enabled"] is True
    assert comp.should_accept(job, depth=0, max_depth=100) is True
    assert comp.explain(job)["component"] == "dlq"


def test_dlq_incr_and_history() -> None:
    comp = Dlq()
