...
1. Open **AfterQuery → Repositories → allot → Environments**.
...
- `WORKDIR /app`
- Pinned installs via `/app/requirements.txt`
- Editable install of `allot` from `/app` (`pip install -e .`)
- No `COPY`, no `ADD` from URLs, no `git clone`, no `|| true`
